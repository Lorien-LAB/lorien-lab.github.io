...
Prove that `(aₙ)` converges and find its limit. Do not begin by assuming the infinite nested radical has a value.
...
For monotonicity, verify `a₂ > a₁` and use that `x ↦ √(2 + x)` is strictly increasing.
...
Prove `aₙ < 2` by induction: if `aₙ < 2`, then `aₙ₊₁ = √(2 + aₙ) < √4 = 2`.
...
Assume `aₙ > aₙ₋₁`. Since `x ↦ √(2 + x)` is strictly increasing,

```text
aₙ₊₁ = √(2 + aₙ) > √(2 + aₙ₋₁) = aₙ
```

By induction, `(aₙ)` is increasing.

### Upper bound by induction

The base case is `a₁ = √2 < 2`. If `aₙ < 2`, then

```text
aₙ₊₁ = √(2 + aₙ) < √(2 + 2) = 2
```

Thus `aₙ < 2` for every `n`, so the sequence is bounded above by `2`.

### Convergence

The sequence is increasing and bounded above. By bounded monotone convergence, it converges to some real `L` with `0 < L ≤ 2`.
...
- Writing `L = √(2 + L)` before proving that `(aₙ)` converges.
...
For `aₙ₊₁ = √(c + aₙ)` with `c > 0`, seek a positive invariant interval and identify the positive root of `L² − L − c = 0` only after convergence.
