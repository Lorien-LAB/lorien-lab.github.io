...
Multiply and divide by the conjugate `√(x² + 5x) + x`.
...
The numerator becomes `(x² + 5x) − x² = 5x`. On the positive tail divide both numerator and denominator by the positive number `x`.
...
We cannot subtract `+∞ − (+∞)`: infinity is not an ordinary number, and this is an indeterminate difference. Rationalize instead:

```text
√(x² + 5x) − x
= [(√(x² + 5x) − x)(√(x² + 5x) + x)]/[√(x² + 5x) + x].
```
...
- Dividing `√(x² + 5x)` by `x` without noting that the limit is on the positive tail.
...
Explain why a limit toward `−∞` requires separate handling of `|x|` when extracting `x²` from a square root.
