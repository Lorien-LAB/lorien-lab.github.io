...
8. Audit this complete binary tree for candidates A, B, C, and D. The root predicate is “candidate is in {A,B}”: Yes maps to {A,B}, and No maps to {C,D}. On Yes, the second predicate is “candidate is A”: Yes maps to A, and No maps to B. On No, the second predicate is “candidate is C”: Yes maps to C, and No maps to D. The leaf path → conclusion table is `root Yes / second Yes → A; root Yes / second No → B; root No / second Yes → C; root No / second No → D`. Audit coverage and uniqueness, then mutate `root No / second No → D` to `→ C` and identify the duplicate and missing-conclusion failure.
