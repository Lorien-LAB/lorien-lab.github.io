...
test('Questions 4-5 remain independently authored S3-plus canonical problem records', async () => {
  for (const slug of ['ants-crossing-line', 'correlation-matrix-parameter-range']) {
...
test('Question 4 teaches identity swapping / crossing invariance rather than simulating collisions', async () => {
  const text = await readFile(await findProblem('ants-crossing-line'), 'utf8');
...
test('Question 5 maps correlation validity to positive semidefiniteness and checks the admissible rho interval', async () => {
  const text = await readFile(await findProblem('correlation-matrix-parameter-range'), 'utf8');
