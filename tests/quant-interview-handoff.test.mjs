  ...
  if (workstream012.status === 'active') {
    ...
    if (workstream013.status === 'active') {
      ...
      if (workstream014.status === 'active') {
        ...
        const workstream015 = JSON.parse(await readFile(
        ...
        if (workstream015.status === 'active') {
          assert.match(current, /Interview Strategy & Communication.*Interview Preparation/is);
          assert.match(current, /Workstream 015 is active/i);
        } else {
          ...
          if (workstream016.status === 'active') {
            ...
            const workstream017 = JSON.parse(await readFile(
                  ...
                  if (workstream020.status === 'active') {
                    ...
                    if (workstream021.status === 'active') {
                      ...
                      const workstream022 = JSON.parse(await readFile(
                        'src/data/quant-interview/workstreams/logic-brainteasers-discrete-reasoning-red-logical-reasoning-core-022.json',
                        'utf8',
                      ));
