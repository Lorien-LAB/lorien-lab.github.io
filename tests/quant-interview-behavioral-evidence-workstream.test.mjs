...
const notes = [
  ...
  'Red Book 9.9 organization-specific research evidence resolves to the behavioral-evidence Knowledge page without publishing current facts.',
...
test('017 remains durable after 022 advances the corpus', async () => {
  const inputs = await loadMasterDirectoryRepository(process.cwd());
  ...
  const manifest022 = inputs.workstreams.find(({ id }) => /-022$/.test(id));
  ...
  assert.equal(manifest021.status, 'complete');
  assert.match(manifest022.status, /^(?:active|complete)$/);
  assert.equal(inputs.problemSlugs.size, 101);
  assert.equal(inputs.knowledgeSlugs.size, 61);
  assert.equal(getNextPendingItem(inputs.directory)?.key, '150-most-frequently-asked::2.7::theory');
});
